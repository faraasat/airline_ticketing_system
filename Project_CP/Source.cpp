...
string SeatChoose()
{
	...
	//Counting Number of lines previously written in seat file:
	...
	//Deleting all information from seat file
	Del.open("Seat_Details.txt", ios::trunc);
	Del.close();

	//Re-writing whole information:
	Del.open("Seat_Details.txt", ios::trunc);
	for (int i = 0; i < 10; i++)
	{
...
void Display(int c)
{
	...
	//Split struct here returns the number of previously recorded line in booking file:
	Counter = SplitStruct();
	system("cls");
	char choice;
...
void Login(int c)
{
	...
	//to count number of lines in login file:
	while (getline(file, line))
	{
		count++;
	}
	...
	//Collecting all names and passwords:
	file.open("Login_Details.txt");
	for (int j = 0; j < count; j++)
	{
		string line;
...
start:
	...
	//check if high level definition is of correct size exist?
	int anotherCount = 0;
	for (int i = dotNo + 1; i <= Email.length(); i++) {
		anotherCount++;
	}
